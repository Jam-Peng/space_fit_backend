...
function DelClientList({ item, currentUser }) {
  const [openList, setOpenList] = useState(null);  // 開啟會員的索引
  const { setDeleteZero, delDataClient } = useContext(ClientContext)
  ...
  // 將黑名單的會員取回
  const retrieve = (currentUser, public_id) => {
    setDeleteZero(currentUser.token, public_id)
  }

  // 從資料庫刪除會員
  const deleteClient = (currentUser, public_id) => {
    delDataClient(currentUser.token, public_id)
  }
        ...
        <div className="px-2">
          <div className="flex items-center px-1 py-2 text-slate-800/70">
            ...
            <div className="w-2/12 flex space-x-2 items-center">
              <button className="btn btn-success" onClick={ ()=>{retrieve(currentUser, public_id)} }>
                <BsStackOverflow size={18}/>
              </button>
              <button className="btn btn-delete" onClick={()=>{deleteClient(currentUser, public_id)} }>
